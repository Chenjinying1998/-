...
/**
 * 
 * 分页拦截器，用于拦截�?要进行分页查询的操作，然后对其进行分页处理�?? 利用拦截器实现Mybatis分页的原理：
 * 要利用JDBC对数据库进行操作就必须要有一个对应的Statement对象
 * ，Mybatis在执行Sql语句前就会产生一个包含Sql语句的Statement对象，�?�且对应的Sql语句
 * 是在Statement之前产生的，�?以我们就可以在它生成Statement之前对用来生成Statement的Sql语句下手�?
 * 在Mybatis中Statement语句是�?�过RoutingStatementHandler对象的prepare方法生成的�??
 * 
 * �?以利用拦截器实现Mybatis分页的一个�?�路就是拦截StatementHandler接口的prepare方法�?
 * 然后在拦截器方法中把Sql语句改成对应的分页查询Sql语句�? 之后再调�?
 * StatementHandler对象的prepare方法，即调用invocation.proceed()�?
 * 
 * 对于分页而言，在拦截器里面我们还�?要做的一个操作就是统计满足当前条件的记录�?共有多少�?
...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		// Mybatis在进行Sql语句处理的时候都是建立的RoutingStatementHandler，�?�在RoutingStatementHandler里面拥有�?个StatementHandler类型的delegate属�?�，
		...
		// 又因为Mybatis只有在建立RoutingStatementHandler的时候是通过Interceptor的plugin方法进行包裹的，�?以我们这里拦截到的目标对象肯定是RoutingStatementHandler对象�?
